      ...
      try {
        this
          ...
          .then((res) => {
            ...
            this
            .axios
            .get(`https://graph.facebook.com/${userId}/picture?width=1200&redirect=false`)
            .then((res2) => {
              user.avatar = res2.data.data
              resolve(user)
            })
            .catch(reject)
          })
        ...
        const lives = await Promise
          ...
          .then((lives) => {
            return _.map(_.flatten(lives), (live) => {
              return _.pick(live, ['id', 'creation_time', 'title', 'owner', 'playable_url'])
            })
          })
          .catch(reject)
