...
class Instagram {
  ...
  setupIds (userIds, usingWebAPI) {
    usingWebAPI = usingWebAPI === true
            ...
            .all(_.map(userIds, (userId) => {
              return new Promise((resolve2, reject2) => {
                if (userId.toString().match(/^(\d+)$/)) {
                  ...
                  //   .catch(reject2)
                  this
                  .getUserByUsername(userId, usingWebAPI)
                  .then((user) => {
                    resolve2(this.cache[userId] = user.pk)
                  })
                  .catch(reject2)
                }
              })
  ...
  setupUsernames (userIds, usingWebAPI) {
    usingWebAPI = usingWebAPI === true
    return new Promise(async (resolve, reject) => {
      try {
        if (userIds === undefined) userIds = []
  ...
  getUserByUsername (username, usingWebAPI) {
    usingWebAPI = usingWebAPI === true
    return new Promise(async (resolve, reject) => {
      try {
        const proxy = await this.options.api.getProxyURL()
        ...
        // console.log('getUserByUsername', username, { usingWebAPI })
        if (usingWebAPI) {
          this
            .client
            .request
            ...
            .then((res) => {
              // console.log('res', res)
              const item = _.find(res.body.users, (item) => {
