...
class API {
  ...
  async getProxy () {
    console.log('options.proxy', this.options.proxy)
    if (this.options.proxy === false || this.options.proxy === undefined || _.isEqual(this.options.proxy, {
      host: null,
      port: null,
      auth: {
        user: null,
      ...
      if (this.options.proxy.auth && this.options.proxy.auth.user && this.options.proxy.auth.pass) {
        if (this.options.webshare && this.options.webshare.shuffle) {
          const users = this.options.proxy.auth.user.split('-')
          const user = `${users[0]}-${_.sample(_.shuffle(_.times(this.options.webshare.amount)))}`
          this.proxy = _.merge(
            {},
            this.options.proxy,
            {
              auth: {
                user,
                pass: this.options.proxy.auth.pass
              }
            }
          )
        } else if (this.options.webshare && this.options.webshare.rotate) {
          if (this.options.webshare.index > this.options.webshare.amount) {
            this.options.webshare.index = 1
          }
          const users = this.options.proxy.auth.user.split('-')
          const user = `${users[0]}-${this.options.webshare.index++}`
          this.proxy = _.merge(
            {},
            this.options.proxy,
            {
